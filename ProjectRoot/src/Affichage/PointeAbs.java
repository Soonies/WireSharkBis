...
public abstract class PointeAbs implements Pointe {
  ...
  public PointeAbs(String portBin, String addresseIpBin) {// tous les deux en binaire
    ...
    if (port.length() != 5) {
      throw new IllegalArgumentException("le port ne respecte pas le bon format de longueur 5");
    }
    if (addresseIp.length() != 5) {
      throw new IllegalArgumentException("l'ip' ne respecte pas le bon format de longueur 15");
    }
    this.port = port;
